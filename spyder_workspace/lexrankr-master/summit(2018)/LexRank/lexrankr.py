...
class SentenceCorpus(TextCorpus):
    ...
    def __init__(self, sentences, no_below=3, no_above=0.8, max_size=None):
        ...
        #my
        self.temp_dict = Dictionary(self.get_texts(), prune_at=None)
        self_bows=self.bows
...
class LexRank(object):
    ...
    def __init__(self, similarity='cosine', decay_window=20, 
                 decay_alpha=0.25, clustering='dbscan', tagger='twitter', 
                 useful_tags=['Noun', 'Verb', 'Adjective', 'Determiner',
                 'Adverb', 'Conjunction', 'Josa', 'PreEomi', 'Eomi', 'Suffix', 
                 'Alpha', 'Number'], delimiters=['. ', '\n', '.\n'],
                  min_token_length=2, stopwords=stopwords_ko,
                  no_below_word_count=2, no_above_word_portion=0.85, 
                  max_dictionary_size=None, min_cluster_size=2, 
                  similarity_threshold=0.85, matrix_smoothing=False, 
                  n_clusters=None, compactify=True, **kwargs):
        
        self.decay_time = 0
        self.simcosine_time = 0
        self.cpu_num = multiprocessing.cpu_count()//2
        # KW: use dense vectors
        self.sentences_dense = {}
        self.sentence_tfidfID_max = 0
        ...
        if similarity == 'cosine':
            ...
            # KW : self.uniform_sim 을 새로 정의한 self._kw_sim_cosine_dense로 재정의
            self.uniform_sim = self._kw_sim_cosine_dense
        ...
        # KW: use _kw_sim_cosine_dense instead of sim_cosine function
        self.kw_sim = lambda sentence1, sentence2: self.decay(sentence1, sentence2) * self._kw_sim_cosine_dense(sentence1, sentence2)
    ...
    def summarize(self, text):
        ...
        print("num_sentence = %d" % self.num_sentences + "\n")
        # Generate cosine similarity matrix
        start_time=time.time()
        print("     <LEXRANK> build_matrix")
        
        # KW: sparse vector -> dense vector
        self._kw_sparse2dense()
    
        # KW: use multi-processing
        self._kw_build_matrix()
        
        print("     %.02f sec" % (time.time() - start_time))
        print("decay time: %.02f sec " % self.decay_time)
        print("simcosine time: %.02f sec" % self.simcosine_time + "\n")
    ...
    # KW: use dense vectors
    # KW: 기존의 self.vectorizer.fit_transform을 제거하고 생성된 sentences_dense vector를 이용하여 dot product 계산
    def _kw_sim_cosine_dense(self, sentence1, sentence2):
        if sentence1 == sentence2:
            return 1        
        
        start_time=time.time()
        
        dotResult = self.sentences_dense[sentence1.index].dot(self.sentences_dense[sentence2.index])
        return dotResult
    ...
    def decay(self, sentence1, sentence2):
        ...
        powResult = math.pow(closeness, self.decay_alpha)
        self.decay_time += time.time() - start_time
    ...
    def _inject_tfidfs(self):
        ...
        for index in range(self.num_sentences):
            ...
            # KW: 모든 sentence들 중에 가장 큰 key 값을 저장            
            if not self.sentences[index].tfidf :
                continue
            else :
                maxtemp = max(self.sentences[index].tfidf)[0]
            
            # KW: 큰 key 값이 들어올 때 마다 sentence_tfidfID_max 값 업데이트
            if(self.sentence_tfidfID_max < maxtemp ):
                self.sentence_tfidfID_max = maxtemp 
            
   
    # KW: generate dense vectors    
    def _kw_sparse2dense(self):
        for sentence in self.sentences:
            sentence_tfidf = {word_id: tfidf for word_id, tfidf in sentence.tfidf}
            temp_dense = []
            
            # KW: _inject_tfidfs()에서 구한 sentence_tfidfID_max을 이용해 밀집 벡터 생성
            # KW: key 값을 갖고 있으면 해당하는 key 값을 , 아니면 0을 삽입
            for i in range(self.sentence_tfidfID_max):
                if i in sentence_tfidf:
                    temp_dense.append(sentence_tfidf[i])
                else :
                    temp_dense.append(0.)
            self.sentences_dense[sentence.index] = np.array(temp_dense)
    
    # KW: sub function for CPU-multi processing 
    def _kw_build_matrix_MPsub(self, startIdx, endIdx, q):
        
        sentence_part = []
        
        for i in range(startIdx, endIdx):
            sentence_part.append(self.sentences[i])
        
        matrix = np.zeros((len(sentence_part), self.num_sentences))
               
        sentence1_idx = -1                         
        for sentence1 in sentence_part:
            sentence1_idx = sentence1_idx + 1
            for sentence2 in self.sentences:
               matrix[sentence1_idx, sentence2.index] = self.kw_sim(sentence1, sentence2)
               
        
        q.put(matrix)
    
    # KW: use CPU-multi processing    
    def _kw_build_matrix(self):
        
        q_list = []
        proc_list = []
        bounds = 0
		
        # KW: split the sentence set for multi-processing
        sizeOfpartition = self.num_sentences//self.cpu_num
        
        for i in range (0,self.cpu_num):
            q_list.append(Queue())
            if i!=(self.cpu_num-1):
                proc_list.append(Process(target=self._kw_build_matrix_MPsub, args=(bounds, bounds + sizeOfpartition ,q_list[i])))
            else:
                proc_list.append(Process(target=self._kw_build_matrix_MPsub, args=(bounds, self.num_sentences ,q_list[i])))
            bounds =  bounds + sizeOfpartition
            
        for p in proc_list:
            p.start()
        
        merged_matrix = q_list[0].get()
        for i in range (1,self.cpu_num):
            merged_matrix = np.concatenate((merged_matrix, q_list[i].get()))
        
        self.matrix = merged_matrix
        
        for q, p in zip(q_list, proc_list) :
            q.close()
            p.join()        
    ...
    def _clustering(self):
        ...
        self_clusters=self.clusters
        self._clustered()
        
    # KW: sub function for CPU-multi processing  
    def _kw_compactify_MPsub(self, cluster, cluster_size, startIdx, endIdx, q):
        compact_cluster = []
        for i in range(startIdx, endIdx):
            ...
            for j in range(i + 1, cluster_size):
                if cluster[j].duplicated:
                    continue
                if self._kw_sim_cosine_dense(cluster[i], cluster[j]) > self.similarity_threshold:
                    cluster[j].duplicated = True
            compact_cluster.append(cluster[i])
        q.put(compact_cluster)
    
       
    # KW: use CPU-multi processing
    def _kw_compactify(self):
        clusters = []
        for cluster in self.clusters:
            ...
            # KW: split the sentence set for multi-processing
            sizeOfpartition = cluster_size//self.cpu_num
            
            for i in range (0,self.cpu_num):
                q_list.append(Queue())
                if i!=(self.cpu_num-1):
                    proc_list.append(Process(target=self._kw_compactify_MPsub, args=(cluster, cluster_size, bounds, bounds + sizeOfpartition ,q_list[i])))
                else:        
                    proc_list.append(Process(target=self._kw_compactify_MPsub, args=(cluster, cluster_size, bounds, cluster_size ,q_list[i])))
            ...
            for i in range (0,self.cpu_num):
                compact_cluster = compact_cluster + q_list[i].get()
    ...
    def keywords(self):
        ...
        sorted_tokens=sorted(result,key=lambda k : result[k],reverse=True)
        
        wordList=[]
        
        sorted_tokens_list=sorted_tokens.keys()
        for word in sorted_tokens_list:
