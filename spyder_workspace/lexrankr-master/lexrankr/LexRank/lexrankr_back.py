...
class LexRank(object):
    ...
    def _kw_sim_cosine(self, sentence1, sentence2):
        ...
        '''
        linalg.init()
        v1=np.asarray(self.sentences_dense[sentence1.index],np.float64)
        v2=np.asarray(self.sentences_dense[sentence2.index],np.float64)
        v1_gpu = gpuarray.to_gpu(v1)
        v2_gpu = gpuarray.to_gpu(v2)
        
        dotResult_gpu = linalg.dot(v1_gpu,v2_gpu)
        return dotResult_gpu
        '''       
        return dotResult
    ...
    def _sim_cosine(self, sentence1, sentence2):
        ...
        self.simcosine_time += time.time() - start_time
                
        
        # Original
        
        dotResult = vector1.dot(vector2)
        
        return dotResult
        '''
        
        linalg.init()
        v1=np.asarray(vector1,np.float64)
        v2=np.asarray(vector2,np.float64)
        v1_gpu = gpuarray.to_gpu(v1)
        v2_gpu = gpuarray.to_gpu(v2)
    ...
    def _inject_tfidfs(self):
        for index in range(self.num_sentences):
            ...
            sentence_tfidf = self.sentences[index].tfidf
    ...
    def _kw_build_matrix_MPsub(self, f_idx, s_idx, q):
        
        sentence_part = []
        
        for i in range(f_idx,s_idx):
        ...
        for sentence1 in sentence_part:
            ...
            for sentence2 in self.sentences:
               matrix[sentence1_idx, sentence2.index] = self.sim(sentence1, sentence2)
    ...
    def _kw_build_matrix(self):
        ...
        proc_list = []
        bounds = 0
        sentence_partition = self.num_sentences//self.cpu_num
        
        for i in range (0,self.cpu_num):
            q_list.append(Queue())
            if i!=(self.cpu_num-1):
                proc_list.append(Process(target=self._kw_build_matrix_MPsub, args=(bounds, bounds + sentence_partition ,q_list[i])))
            else:
                proc_list.append(Process(target=self._kw_build_matrix_MPsub, args=(bounds, self.num_sentences ,q_list[i])))
            bounds =  bounds + sentence_partition
    ...
    # pre-optimization
    def _compactify_sub(self, cluster, cluster_size, f_idx, s_idx, q):
        compact_cluster = []
        for i in range(f_idx, s_idx):
    ...
    def _compactify(self):
        ...
        for cluster in self.clusters:
            ...
            proc_list = []
            bounds = 0
            sentence_partition = cluster_size//self.cpu_num
            
            for i in range (0,self.cpu_num):
                q_list.append(Queue())
                if i!=(self.cpu_num-1):
                    proc_list.append(Process(target=self._compactify_sub, args=(cluster, cluster_size, bounds, bounds + sentence_partition ,q_list[i])))
                else:
                    proc_list.append(Process(target=self._compactify_sub, args=(cluster, cluster_size, bounds, cluster_size ,q_list[i])))
